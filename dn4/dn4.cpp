...
int main() {
  ...
  auto start_time = std::chrono::high_resolution_clock::now();
      ...
      for (int ii = 0; ii < 2000; ii++) { // dodamo zanko čez vse zanke
        for (int j = 0; j < n; j++) { // n->tok je elementov
            double d = b[j];
            double ajj_inv = 1.0 / A[j][j]; //izračun inverzne matrike
            for (int i = 0; i < n; i++) { //še ena zanka skozi matriko A
                if (i != j) { //nočemo uporabiti diagonale
                    d -= A[j][i] * T[i];
                }
            }
            T[j] = d * ajj_inv; //posodobitev T-ja
      }
    } 
...
/*
Pri Gauss-Seidlovi ne uporabimo paralelizacije zarad tega, ker je metoda zaporedne narave. 
Posodobitev vrednosti posamezne spremenljivke T, zahteva uporabo že posodobljenih vrednosti T-ja iz iste iteracije. 
Zaradi tega je izračun T-ja odvisen od rezultatov prejšnjih spremenljivk, 
kar preprečuje, da lahko vse spremenljivke hkrati posodobili neodvisno. 
*/